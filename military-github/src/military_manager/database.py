...
from military_manager.config import DATABASE_URL
...
def get_engine():
    """Get or create the SQLAlchemy engine with SQLite optimizations."""
    global _engine
    if _engine is None:
        ...
        def set_sqlite_pragma(dbapi_connection, connection_record):
            ...
            cursor.execute("PRAGMA synchronous=NORMAL")  # safe with WAL, ensures durability
...
def _run_migrations(engine) -> None:
    ...
    try:
        ...
        if "submitted_by_user_id" not in req_cols:
            cur.execute("ALTER TABLE requests ADD COLUMN submitted_by_user_id INTEGER REFERENCES users(id)")
        if "assigned_to_user_id" not in req_cols:
            cur.execute("ALTER TABLE requests ADD COLUMN assigned_to_user_id INTEGER REFERENCES users(id)")

        # ── period_soldiers table ──
        cur.execute("PRAGMA table_info(period_soldiers)")
        ps_cols = {row[1] for row in cur.fetchall()}
        if "is_irrelevant" not in ps_cols:
            cur.execute("ALTER TABLE period_soldiers ADD COLUMN is_irrelevant BOOLEAN DEFAULT 0")
        if "is_attached" not in ps_cols:
            cur.execute("ALTER TABLE period_soldiers ADD COLUMN is_attached BOOLEAN DEFAULT 0")
        if "is_student" not in ps_cols:
            cur.execute("ALTER TABLE period_soldiers ADD COLUMN is_student BOOLEAN DEFAULT 0")
        if "student_short_service" not in ps_cols:
            cur.execute("ALTER TABLE period_soldiers ADD COLUMN student_short_service BOOLEAN DEFAULT 0")

        # ── soldier_constraints table ──
        cur.execute("PRAGMA table_info(soldier_constraints)")
        sc_cols = {row[1] for row in cur.fetchall()}
        if "blocked_tasks" not in sc_cols:
            cur.execute("ALTER TABLE soldier_constraints ADD COLUMN blocked_tasks TEXT")

        # ── daily_status table ──
        cur.execute("PRAGMA table_info(daily_status)")
        ds_cols = {row[1] for row in cur.fetchall()}
        if "notes" not in ds_cols:
            cur.execute("ALTER TABLE daily_status ADD COLUMN notes TEXT")
