...
def _render_slot_editor(slots_key: str, initial_slots: list[dict] | None = None):
    ...
    if chosen_template != "—":
        if st.button("🔄 טען תבנית", key=f"{slots_key}_load_tmpl"):
    ...
    for i, slot in enumerate(slots):
        ...
        with col_del:
            st.write("")
            st.write("")
            if st.button("🗑️", key=f"{slots_key}_del_{i}"):
                to_remove = i
    ...
    # Add slot button
    if st.button("➕ הוסף תפקיד", key=f"{slots_key}_add"):
...
def render():
            ...
            for task in tasks:
                ...
                with st.expander(
                    ...
                    if (nc != bool(current_nc) or rtype != current_rtype or rconf != current_rconf):
                        if st.button("💾 שמור הגדרות חילוף", key=f"save_rot_{task.id}"):
                            update_task(
                                task.id,
                                non_continuous=nc,
                                rotation_type=rtype,
                    ...
                    if st.button("💾 שמור תפקידים", key=f"save_slots_{task.id}"):
                    ...
                    with c_toggle:
                        ...
                        if st.button(f"🔄 {label}", key=f"toggle_{task.id}"):
    ...
    with tab_add:
        ...
        if st.button("✅ צור משימה", key="create_task_btn"):
            if not name:
