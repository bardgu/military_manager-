...
def _render_driver_list(pid: int):
    ...
    if filtered:
        ...
        if sel_remove != "—":
            ...
            if driver_id and st.button("🗑️ הסר", key="do_remove_driver"):
...
def _render_propose_tab(pid: int):
    ...
    if potential:
        ...
        for sol in potential:
            ...
            with col4:
                if st.button("➕ הצע", key=f"pot_propose_{sol['soldier_id']}"):
        ...
        # Bulk propose all
        if st.button("📥 הצע את כולם", key="propose_all_potential"):
            for sol in potential:
                propose_driver(
                    pid, sol["soldier_id"],
                    proposed_by=st.session_state.get("commander_name", ""),
    ...
    if non_drivers:
        ...
        if sel != "—":
            ...
            if sid and st.button("➕ הצע כנהג", key="do_manual_propose"):
                final_vtype = custom_vtype.strip() if custom_vtype and custom_vtype.strip() else vtype
                propose_driver(
                    pid, sid,
                    proposed_by=st.session_state.get("commander_name", ""),
    ...
    for d in pending:
        with st.container():
            ...
            with col_actions:
                ...
                with c_approve:
                    if st.button("✅ אשר", key=f"approve_{d['id']}"):
                        approver = st.session_state.get("commander_name", "סמ\"פ")
                        approve_driver(d["id"], approved_by=approver)
                        st.success(f"{d['full_name']} אושר!")
                        st.rerun()
                with c_reject:
                    if st.button("❌ דחה", key=f"reject_{d['id']}"):
                        approver = st.session_state.get("commander_name", "סמ\"פ")
