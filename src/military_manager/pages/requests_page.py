...
def render():
            ...
            for req in pending:
                ...
                with st.expander(f"📨 {name} — {req['type']} ({req['start_date']} — {req['end_date']})"):
                    ...
                    if _can_decide(req, current_user):
                        ...
                        c1, c2 = st.columns(2)
                        with c1:
                            if st.button("✅ אשר", key=f"approve_{req['id']}", disabled=readonly):
                                ...
                                st.rerun()
                        with c2:
                            if st.button("❌ דחה", key=f"reject_{req['id']}", disabled=readonly):
                                decided_name = current_user.get("display_name", "") if current_user else ""
    ...
    with tab_new:
        with st.form("new_request_form"):
            ...
            if st.form_submit_button("📨 שלח בקשה", disabled=readonly):
