...
from military_manager.config import DEFAULT_STATUSES, IS_POSTGRES
...
# Backup service is SQLite-only — import conditionally
if not IS_POSTGRES:
    from military_manager.config import DATABASE_PATH
...
def render():
    ...
    if IS_POSTGRES:
        # No backup/system tabs in cloud mode (Supabase manages the DB)
        tab_status, tab_groups, tab_alerts, tab_irrelevant, tab_import, tab_system = st.tabs([
            "🏷️ סטטוסים", "📊 קבוצות סטטוס", "🚨 ספים והתראות",
            "🚫 לא רלוונטיים", "📥 ייבוא מלא", "🖥️ מערכת"
        ])
    else:
...
def _render_system_info():
    ...
    if IS_POSTGRES:
        # Cloud mode — show simplified info using SQLAlchemy queries
        st.info("🌐 מסד נתונים: **Supabase (PostgreSQL)** — גיבויים מנוהלים אוטומטית ע\"י הספק")
        st.markdown("#### 📈 סטטיסטיקות נתונים")
        _render_db_record_counts()
    else:
        # Local mode — full SQLite stats
        stats = get_db_stats()
        if stats.get("exists"):
    ...
    from military_manager.components.auth import get_effective_company_id
    ...
    if periods:
        ...
        if st.button("🔄 הפעל תקופה"):
            ...
            activate_period(period_id, company_id)
    ...
    # --- DANGER ZONE: DB Reset (מ"פ only, SQLite only) ---
    if not IS_POSTGRES:
        st.markdown("---")
...
def _render_db_record_counts():
    """Show record counts using SQLAlchemy (works on both SQLite and PostgreSQL)."""
    from military_manager.database import (
        get_session, Soldier, ReservePeriod, PeriodSoldier,
        DailyStatus, Task, TaskSlot, ShiftAssignment,
        SoldierConstraint, PeriodDriver, AuditLog,
    )
    from sqlalchemy import func, select as sa_select

    table_labels = [
        (Soldier, "חיילים"),
        (ReservePeriod, "תקופות"),
        (PeriodSoldier, "שיבוצים לתקופות"),
        (DailyStatus, "סטטוסים יומיים"),
        (Task, "משימות"),
        (TaskSlot, "עמדות"),
        (ShiftAssignment, "שיבוצים למשמרות"),
        (SoldierConstraint, "אילוצים"),
        (PeriodDriver, "נהגים"),
        (AuditLog, "רשומות לוג"),
    ]
    cols = st.columns(5)
    with get_session() as session:
        for i, (model, label) in enumerate(table_labels):
            count = session.execute(sa_select(func.count()).select_from(model)).scalar() or 0
            with cols[i % 5]:
                st.metric(label, f"{count:,}")
