...
def render():
    ...
    with tab_list:
        _render_user_list(sub_units, companies)

    with tab_add:
        _render_add_user(sub_units, companies)


def _render_user_list(sub_units: list[str], companies: list[dict]):
    ...
    for u in users:
        ...
        comp_name = company_map.get(u.get("company_id"), "")
        comp_tag = f" | {comp_name}" if comp_name else ""

        with st.expander(f"{active_tag} {u['display_name']} — {role_label}{sub}{comp_tag}"):
...
def _render_add_user(sub_units: list[str], companies: list[dict]):
    """Add new user form."""
    with st.form("add_user_form"):
        ...
        # Company selector
        company_names = ["ללא פלוגה"] + [c["name"] for c in companies]
        company_choice = st.selectbox("פלוגה", company_names)
        selected_company_id = None
        if company_choice != "ללא פלוגה":
            selected_company_id = next(
                (c["id"] for c in companies if c["name"] == company_choice), None
            )
