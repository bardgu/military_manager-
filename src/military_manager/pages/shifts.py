...
def _render_manual_assignment(pid: int, p_start, p_end):
    ...
    if not eligible_officers:
        ...
        if duty_selection != "—":
            ...
            if duty_sid and st.button("✅ קבע קצין תורן", key="set_duty", disabled=readonly):
...
def _render_auto_assignment(pid: int, p_start, p_end):
    ...
    if mode == "📅 יום בודד":
        ...
        if st.button("🚀 הפעל שיבוץ אוטומטי", type="primary", key="run_auto_single", disabled=readonly):
