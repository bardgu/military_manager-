...
def create_period(name: str, start_date: date, end_date: date,
                  ...
                  company_id: int | None = None) -> ReservePeriod:
...
def get_active_period(company_id: int | None = None) -> ReservePeriod | None:
    """Get the currently active reserve period, optionally filtered by company."""
    with get_session() as session:
        stmt = (
        ...
        if company_id is not None:
            stmt = stmt.where(ReservePeriod.company_id == company_id)
        stmt = stmt.order_by(ReservePeriod.start_date.desc()).limit(1)
        return session.execute(stmt).scalar_one_or_none()


def get_all_periods(company_id: int | None = None) -> list[ReservePeriod]:
    """Get all reserve periods, optionally filtered by company."""
    with get_session() as session:
        stmt = select(ReservePeriod)
        if company_id is not None:
            stmt = stmt.where(ReservePeriod.company_id == company_id)
        stmt = stmt.order_by(ReservePeriod.start_date.desc())
...
def activate_period(period_id: int, company_id: int | None = None) -> None:
    """Set a period as active (deactivate others in the same company)."""
    with get_session() as session:
        # Deactivate all periods in the same company (or globally if no company)
        deactivate_stmt = update(ReservePeriod).values(is_active=False)
        if company_id is not None:
            deactivate_stmt = deactivate_stmt.where(ReservePeriod.company_id == company_id)
