...
from military_manager.config import DATA_DIR, IS_POSTGRES
from military_manager.logger import log_action

# ─── Guard: backup service is SQLite-only ──────────────────────
if IS_POSTGRES:
    # When using PostgreSQL (Supabase), backups are managed by the provider.
    # Import DATABASE_PATH only when not using Postgres.
    DATABASE_PATH = None
else:
