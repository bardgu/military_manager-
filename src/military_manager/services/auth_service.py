...
def create_user(username: str, password: str, display_name: str,
                role: str = "viewer", sub_unit: str | None = None,
                company_id: int | None = None) -> User | None:
...
def get_all_users(company_id: int | None = None) -> list[dict]:
    """Get all users, optionally filtered by company."""
    with get_session() as session:
        stmt = select(User).order_by(User.role, User.display_name)
