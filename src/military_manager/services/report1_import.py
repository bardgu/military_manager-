...
_SA_KEY_INFO: dict | None = None   # in-memory credentials from Streamlit Secrets


def _load_sa_from_secrets() -> dict | None:
    """Try to load service-account JSON from Streamlit Secrets."""
    global _SA_KEY_INFO
    if _SA_KEY_INFO is not None:
        return _SA_KEY_INFO
    try:
        import streamlit as st
        # Streamlit Secrets: stored under [service_account] section as key-value pairs
        sa = dict(st.secrets.get("service_account", {}))
...
def _find_sa_key() -> Path | None:
    """Locate the service-account JSON key file on disk."""
    global _SA_KEY_PATH
...
def fetch_google_sheet_as_excel(url: str) -> BytesIO:
    ...
    sa_key = _find_sa_key()
    sa_info = _load_sa_from_secrets()

    # ── Method 1: Service Account (private sheets) ──
    if sa_key or sa_info:
        try:
            ...
            # Prefer in-memory credentials (Streamlit Secrets), fall back to file
            if sa_info:
